...
public class WSClient implements IWebsocket {
    ...
    private List<WSListener> listenes = new CopyOnWriteArrayList<WSListener>();
                    ...
                    while (!Thread.interrupted()) {
                        while ((read = is.read(buf)) != -1) {
                            if (read > buffer.limit() - buffer.position()) {
                                ...
                                buffer = Buffers.appendBuffers(mm, newBuf, buffer);
                            }
                            ...
                            if (message.frameType == WSFrameType.CLOSE) {
                                ...
                                if (data.length < 2) {
                                    onDisconnect(NORMAL, "normal closed");
                                    break;
                                }
                                int code = data[0] << 8 | data[1];
                                onDisconnect(code, new String(data, 2, data.length - 2));
                                break;
                            }
    ...
    public void send(Object data) {
        ...
        if (data instanceof byte[]) {
            writeAndFlush(WSMessage.create(WSFrameType.BIN, (byte[]) data, mask));
            return;
        }
    ...
    public void onDisconnect(int code, String info) {
        ...
        for (WSListener listener : listenes) {
